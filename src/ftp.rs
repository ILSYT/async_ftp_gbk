...
impl FtpStream {
        ...
        R: AsyncBufRead + Unpin,
    {
        ...
        let mut lines_stream = data_stream.split(0xa);
    ...
    async fn write_str<S: AsRef<str>>(&mut self, command: S) -> Result<()> {
        ...
        let cmd = encoding_rs::GBK.encode(command.as_ref());

        let stream = self.reader.get_mut();
        stream
            .write_all(cmd.0.as_ref())
            .await
            .map_err(FtpError::ConnectionError)
    }
    ...
    pub async fn read_response_in(&mut self, expected_code: &[u32]) -> Result<Line> {
        ...
        let mut line = encoding_rs::GBK.decode(line.as_ref()).0.to_string();
